...
export default function Navbar() {
  ...
  // Helper to check if a nav link is the "Who We Are" entry
  const isWhoWeAre = (linkName: string) => {
    // We compare the raw name (could be a translation key). The actual display name is safeT(linkName)
    // But we want to detect the "Who We Are" link regardless of language.
    // Since the key is 'Who We Are'? Actually in navLinks we used 'Who We Are' as a literal.
    // To be safe, check if the translated text equals "Who We Are" or the key is 'Who We Are'.
    // We'll check the raw name.
    return linkName === 'Who We Are';
  };
            ...
            <div className="flex items-center justify-end gap-3">
              ...
              <nav className="hidden md:flex items-center gap-2 lg:gap-4 ml-auto">
                {navLinks.map((link) => {
                  ...
                  const isWhoWeAreLink = isWhoWeAre(link.name);
                    ...
                    return (
                      ...
                      >
                        <button
                          ...
                          style={isWhoWeAreLink ? { fontFamily: "'Comic Sans MS', cursive" } : {}}
                        >
                          {safeT(link.name)}
                  ...
                  // For non-submenu links
                  const isWhoWeAreLinkSimple = isWhoWeAre(link.name);
                  return (
                    <Link
                      key={link.name}
                      to={link.path}
                      ...
                      style={isWhoWeAreLinkSimple ? { fontFamily: "'Comic Sans MS', cursive" } : {}}
                    >
                      {safeT(link.name)}
                      <span
                        className={`absolute -bottom-1 left-0 h-[2px] bg-red-600 transition-all ${
